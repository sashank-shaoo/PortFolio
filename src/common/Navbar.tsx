  ...
  return (
    <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-4xl">
      <div className="flex justify-between items-center bg-zinc-900/70 backdrop-blur-md border border-white/10 rounded-full px-6 py-3 shadow-2xl relative">
        ...
        >
          {isOpen ? <X className="w-5 h-5" /> : <EllipsisVertical className="w-5 h-5" />}
        </button>
      ...
      >
        <div className="bg-zinc-900/90 backdrop-blur-lg border border-white/10 rounded-2xl p-4 shadow-2xl flex flex-col gap-4 text-md font-medium text-zinc-300 items-center">
          <a href="#" onClick={() => setIsOpen(false)} className="hover:text-white transition-colors w-full text-center py-2 border-b border-white/5 last:border-0">Home</a>
          <a href="#about" onClick={() => setIsOpen(false)} className="hover:text-white transition-colors w-full text-center py-2 border-b border-white/5 last:border-0">About</a>
          <a href="#skills" onClick={() => setIsOpen(false)} className="hover:text-white transition-colors w-full text-center py-2 border-b border-white/5 last:border-0">Skills</a>
          <a href="#projects" onClick={() => setIsOpen(false)} className="hover:text-white transition-colors w-full text-center py-2 border-b border-white/5 last:border-0">Projects</a>
        </div>
      </div>
    </nav>
  )
